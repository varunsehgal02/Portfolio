...
import { trackAboutPopupOpen } from '@/lib/analytics';
import './MagicBento.css';

const DEFAULT_PARTICLE_COUNT = 12;
const DEFAULT_SPOTLIGHT_RADIUS = 300;
const DEFAULT_GLOW_COLOR = '59, 109, 224';
...
const defaultCardData = [
...

    {
        ...
        description: '80% repeat business rate across diverse sectors',
        label: '⭐ Results'
    },
    {
        color: '#060a1a',
        title: 'Brand Visibility +35%',
        description: 'Amplified engagement by 45% across digital channels',
        label: '📈 Impact'
    },
    {
        color: '#060a1a',
        title: 'B.Tech CSE (2023-27)',
        description: 'Jaypee University of Engineering and Technology, Gwalior',
        label: '🎓 Education'
    }
];
                ...
                {processedCards.map((card, index) => {
                    ...
                    const cardProps = {
                        ...
                        style: {
                            ...
                            '--media-opacity': card.mediaOpacity ?? 0.4,
                        },
                    };
                    return (
                        <ParticleCard
                            ...
                            onClick={() => {
                                ...
                                trackAboutPopupOpen(card.title || 'Untitled Card');
                            }}
                        >
                            {card.mediaType === 'image' && card.mediaUrl ? (
                                <div className="magic-bento-card__media" aria-hidden="true">
                                    <img src={card.mediaUrl} alt="" className="magic-bento-card__media-content" />
                                </div>
                            ) : null}
                            {card.mediaType === 'video' && card.mediaUrl ? (
                                <div className="magic-bento-card__media" aria-hidden="true">
                                    <video
                                        ...
                                        preload="metadata"
                                        className="magic-bento-card__media-content"
                                    />
                                </div>
                            ) : null}
                            ...
                            {Array.isArray(card.points) && card.points.length > 0 ? (
                                <div className="magic-bento-card__meta">
                                    {card.points.slice(0, 4).map((point, pIdx) => (
                                        <span key={`${point}-${pIdx}`} className="magic-bento-card__chip">
                                            {point}
                                        </span>
                                    ))}
                                </div>
            ...
            <AnimatePresence>
                {selectedCard ? (
                    ...
                    >
                        <button
                            type="button"
                            className="magic-bento-modal__backdrop"
                            onClick={() => setSelectedCard(null)}
                            aria-label="Close popup"
                        />

                        <motion.div
                            className="magic-bento-modal__panel"
                        ...
                        >
                            <button
                                type="button"
                                className="magic-bento-modal__close"
                                onClick={(e) => {
                                    e.stopPropagation();
                                    setSelectedCard(null);
                                }}
                            ...
                            <div className="magic-bento-modal__media-wrap">
                                {selectedCard.mediaType === 'image' && selectedCard.mediaUrl ? (
                                    <img src={selectedCard.mediaUrl} alt={selectedCard.title || 'Card media'} className="magic-bento-modal__media" />
                                ) : null}
                                {selectedCard.mediaType === 'video' && selectedCard.mediaUrl ? (
                                    <video
                                        ...
                                        playsInline
                                        preload="metadata"
                                        className="magic-bento-modal__media"
                                    />
                                ) : null}
                            </div>

                            <p className="magic-bento-modal__label">{selectedCard.label}</p>
                            <h3 className="magic-bento-modal__title">{selectedCard.popupTitle || selectedCard.title}</h3>
                            <p className="magic-bento-modal__description">{selectedCard.popupDescription || selectedCard.description}</p>

                            {Array.isArray(selectedCard.points) && selectedCard.points.length > 0 ? (
                                <div className="magic-bento-modal__chips">
                                    {selectedCard.points.map((point, idx) => (
                                        <span key={`${point}-${idx}`} className="magic-bento-card__chip">{point}</span>
                                    ))}
                                </div>
                            ) : null}

                            {selectedCard.popupLink ? (
                                <a href={selectedCard.popupLink} target="_blank" rel="noopener noreferrer" className="magic-bento-modal__cta">
                                    {selectedCard.popupLinkLabel || 'Open Link'}
                                </a>
                            ) : null}
                        </motion.div>
                    </motion.div>
