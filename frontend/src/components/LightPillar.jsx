...
const LightPillar = ({
    ...
    useEffect(() => {
        ...
        let eq = quality;
        if (isLowEnd && quality === "high") eq = "medium";
        if (isMobile && quality !== "low") eq = "low";
      ...
      uniform float uRotCos;uniform float uRotSin;uniform float uPillarRotCos;
      uniform float uPillarRotSin;uniform float uWaveSin;uniform float uWaveCos;
      varying vec2 vUv;
      ...
      void main(){
        ...
        const frameTime = 1000 / (eq === "low" ? 30 : 60);
    ...
    if (!webGLSupported) {
        return <div className={className} style={{ mixBlendMode, width: "100%", height: "100%", background: "rgba(0,0,0,0.1)", display: "flex", alignItems: "center", justifyContent: "center", color: "#888" }}>WebGL not supported</div>;
    }

    return <div ref={containerRef} className={className} style={{ mixBlendMode, width: "100%", height: "100%", position: "absolute", top: 0, left: 0 }} />;
};
